...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if platform.system() == "Windows":
            ...
            # If using NMake for generator or building on conda, do not use arch specifier as not
            # supported.
            cmake_generator = os.environ.get("CMAKE_GENERATOR", "")
            if sys.maxsize > 2**32 and not cmake_generator.startswith("NMake") and not conda_build:
                cmake_args += ['-A', 'x64']
            if not conda_build:
                build_args += ['--', '/m']
        else:
